    ...
    if (userDoc) {
        ...
        if (passOk) {
            jwt.sign({
                ...
                if (err) throw err
                res.cookie(res.cookie('token', token).json(userDoc))
            })
        } else res.status(422).json('not ok')
...
app.post('/bookings', async (req, res) => {
    const {place, numOfGuests, name, phone, checkIn, checkOut, price} = req.body
    const userData = await getUserDataFromReq(req)

    Booking.create({
        place, numOfGuests, name, phone, checkIn, checkOut, price, user: userData.id
    }).then((doc) => {
        res.json(doc)
    }).catch((err) => {
        throw err
...
app.get('/booking/:id', async (req, res) => {
    const bookinkId = req.params.id
    const bookingData = await Booking.findById(bookinkId).populate('place')
    res.json(bookingData)
})
